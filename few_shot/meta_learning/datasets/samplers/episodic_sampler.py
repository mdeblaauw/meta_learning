    ...
    def __iter__(self):
        ...
        for _ in range(self.episodes_per_epoch):
            ...
            episode_classes = np.random.choice(
                self.df['class_id'].unique(),
                size=self.k, replace=False
            )

            chosen_class_df = self.df[
                self.df['class_id'].isin(episode_classes)
            ]

            support_k = {k: None for k in episode_classes}
            # Select n*k support samples first
            for k in episode_classes:
                support = self.df[self.df['class_id'] == k].sample(self.n)
                support_k[k] = support
            ...
            for k in episode_classes:
                query = self.df[
                    (
                        ...
                        ~self.df['id'].isin(support_k[k]['id'])
                    )
                ].sample(self.q)
